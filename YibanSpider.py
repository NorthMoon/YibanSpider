...
def ymm():
    print('\n获取易瞄瞄相关网薪....')
    print('将在易瞄瞄进行 发帖*3 评论*3，预计网薪+8')
...
def feeds():
    print('\n获取动态相关网薪.....')
    print('将进行 发布点赞同情*3，动态仅自己可见！预计网薪 16')
...
def blog():
    print('\n获取博客相关网薪....')
    print('将在轻博客进行 发布*3 点赞*3，博客仅自己可见！预计网薪+16')
    global blog_headers
    blog_headers = {
        'User-Agent': 'Mozilla/5.0 (Linux; Android 10; Redmi Note 7 Pro Build/QKQ1.190915.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.138 Mobile Safari/537.36 yiban_android',
        'appversion': '4.7.11',
...
def clear():
    print('\n能扒的网薪已经尽力扒了，下面根据你的需要擦擦屁股')
    choose = input('\n是否删除刚刚发布的动态和博客\n数量：各3条\n权限：仅自己可见\n内容：随机古诗词，注意：删除会扣网薪，反正别人看不见..\n\n确认删除输入 del 并回车，跳过直接回车>')
    if choose == 'del':
        delfeeds()
        delblog()
    choose = input('\n是否删除刚刚发布轻社区话题\n数量：1条\n权限：公开\n内容：随机古诗词\n注意：扣网薪，保留可以为公共群水一下EGPA\n\n确认删除输入 del 并回车，跳过直接回车>')
    if choose == 'del':
        deltopics()
    return
def main():
    ...
    return
#login("18813363680", ".7788920")
#login("13692445368", "z2533083877")
main()
